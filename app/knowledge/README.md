...
分层查询顺序：Steam → 官网/其他商店 → 维基百科 → 百度检索来源页面。
上一层失败或无法匹配会继续下一层，取得可用正文后停止。不自动重试同一请求。
全部失败时记录每层原因；存在歧义则名称待确认，否则补充失败。

官网层通过百度发现页面，只接受 web_sources.py 中 OFFICIAL_HOSTS 指定的商店、
发行商和开发商域名，不凭标题“官网”二字判定。当前支持范围有限，可扩充域名集合。
维基百科使用中英文 Action API，支持简繁转换、重定向和带“游戏”消歧后缀的条目；
消歧页、系列条目及无法识别为游戏的条目不作为单款游戏资料。
最终百度层读取百科和指定游戏参考网站的实际页面，不保存搜索结果摘要。
外部页面的标题和游戏语境必须匹配；不能确定同名作品身份时留待确认。

百度目前在本机可能返回安全验证，程序不会绕过验证，会把该层视为不可访问；
官网发现也依赖百度，受同样限制。维基百科真实读取已验证。网站接口或 HTML 变化
可能影响读取；每次网络请求最多 15 秒（Steam 20 秒）。
